...
public class MainController {
    ...
    public DataModel getHomeDashboard(@RequestParam(name = "uid", required = true) String uid,
     ...
     * @param uid  : 로그인 유저 아이디
     * @param auth : 권한 코드값
     * @return DataModel : 일별/주간별/월별 + 서비스별 CRITICAL 발생 빈도수, 하룻동안의 LOG_PRIORITY 상태
     *         표시 ( CRITICAL, NORMAL )
     */
    @GetMapping("/graph/synthesis")
    public DataModel getSynthesisGraph(@RequestParam(name = "uid", required = true) String uid,
            @RequestParam(name = "auth", required = true) String auth) {
        DataModel params = new DataModel();
        params.putStrNull("uid", uid);
        params.putStrNull("auth", auth);
    ...
    public DataModel getCheckServerGraph(@RequestParam(name = "uid", required = true) String uid,
            @RequestParam(name = "auth", required = true) String auth) {
        DataModel params = new DataModel();
        params.putStrNull("uid", uid);
        params.putStrNull("auth", auth);
    ...
    public DataModel getSefilCareGraph(@RequestParam(name = "uid", required = true) String uid,
            @RequestParam(name = "auth", required = true) String auth) {
        DataModel params = new DataModel();
        logger.info("SefilCarer start : {}", params);
        params.putStrNull("uid", uid);
        params.putStrNull("auth", auth);
    ...
    public DataModel getServiceList(@RequestParam(name = "uid", required = true) String uid,
            @RequestParam(name = "auth", required = true) String auth) {
        DataModel params = new DataModel();
        params.putStrNull("uid", uid);
        params.putStrNull("auth", auth);
